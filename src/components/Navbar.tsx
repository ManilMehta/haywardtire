...
export default function Navbar() {
  ...
  return (
    <nav className="bg-dark sticky top-0 z-50 shadow-lg">
        ...
        <div className="flex h-48 items-center justify-between">
          <Link href="/" className="flex items-center gap-8 shrink-0">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-base text-accent tracking-widest uppercase">
          ...
          <button
            ...
            className="md:hidden rounded-md p-3 text-gray-300 hover:text-white"
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="md:hidden border-t border-dark-light bg-dark pb-6">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              onClick={() => setMobileOpen(false)}
              className={`block px-8 py-4 text-lg font-semibold transition-colors ${
                pathname === link.href
                  ? "bg-primary text-white"
                  : "text-gray-300 hover:text-white hover:bg-dark-light"
              }`}
          ...
          <a
            href="tel:+15104701162"
            className="mx-6 mt-4 flex items-center justify-center gap-3 rounded-full bg-primary px-8 py-4 text-lg font-bold text-white"
          >
            Call (510) 470-1162
          </a>
        </div>
